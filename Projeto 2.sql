...
-- Colunas: status profissionais, leads(%)
...
order by faixa_etária

-- (Query 4) Faixa Salárial dos leads
...
select
	"idade do veículo",
	count(*)::float/(select count(*) from faixa_de_idade_dos_veiculos) as "veículos visitados (%)"
